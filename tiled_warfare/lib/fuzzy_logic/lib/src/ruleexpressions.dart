...
abstract class FuzzyNode {
  late FuzzyNode? parent;
  late Set<FuzzyNode>? children;
  ...
  bool containsVariable(FuzzyVariable variable) {
    if (isLeaf) {
      ...
      return children?.any((node) => node.containsVariable(variable)) ?? false;
    }
  }
...
class _FuzzyAnd extends FuzzyTerm {
  ...
  @override
  num getDegreeOfMembershipWithInputs(List<FuzzyValue> inputs) {
    final num minimum = children?.fold(null, (num? value, FuzzyNode n) {
...
class _FuzzyOr extends FuzzyTerm {
  ...
  @override
  num getDegreeOfMembershipWithInputs(List<FuzzyValue> inputs) {
    final num maximum = children?.fold(null, (num? value, FuzzyNode n) {
          final dom = n.getDegreeOfMembershipWithInputs(inputs);
          //if (value < dom) {
